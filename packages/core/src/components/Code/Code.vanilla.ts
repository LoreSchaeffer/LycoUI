export function initLycoCodes(): void {
    const codes = document.querySelectorAll<HTMLDivElement>('.code:not([data-lyco-initialized])');
    codes.forEach(code => {
...
class LycoCodeController {
    ...
    constructor(element: HTMLDivElement) {
        ...
        this.select = element.querySelector<HTMLSelectElement>('.code__lang-select');
        this.copyBtn = element.querySelector<HTMLButtonElement>('.code__action[aria-label="Copy code"]');
        this.downloadBtn = element.querySelector<HTMLButtonElement>('.code__action[aria-label="Download code"]');
        this.textarea = element.querySelector<HTMLTextAreaElement>('.code__textarea');
        ...
        this._onCopyClick = async () => {
            const codeElement = this.element.querySelector('.code__body pre');
        ...
        this._onTextareaScroll = () => {
            if (this.textarea) {
                const pre = this.element.querySelector<HTMLPreElement>('.code__highlight pre');
                if (pre) {
